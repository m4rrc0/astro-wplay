...
function filterOrganizations(organizationsForm) {
  ...
  const servicesFilter = filterParams.filter(([key,]) => key === 'services').map(([,value]) => value);
  ...
  for (let i = 0; i < organizations.length; i++) {
    if (
      ...
      (servicesFilter && servicesFilter.length && (servicesFilter.length > 1 || servicesFilter[0] !== '') && !servicesFilter.some(service => {
